...
data class PlatformMatch(val app: String, val apps: List<String>)
...
object PlatformRuleStore {
    ...
    val DEFAULT_RULES = listOf(
        ...
        PlatformRule("【闲鱼】", listOf("m.tb.cn", "goofish.com"), "com.taobao.idlefish"),
        PlatformRule("【京东】", listOf("3.cn", "jd.com", "jd.hk"), "com.jingdong.app.mall"),
        PlatformRule("【拼多多】", listOf("yangkeduo.com", "pinduoduo.com"), "com.xunmeng.pinduoduo"),
        PlatformRule("【小红书】", listOf("xhslink.com", "xhslink.cn"), "com.xingin.xhs"),
        PlatformRule("", listOf("douyin.com"), "com.ss.android.ugc.aweme"),
        PlatformRule("", listOf("b23.tv"), "tv.danmaku.bili"),
        PlatformRule("「百度网盘APP 即可获取」", listOf("pan.baidu.com"), "com.baidu.netdisk"),
        PlatformRule("「中国移动云盘APP」", listOf("yun.139.com"), "com.chinamobile.mcloud"),
    ...
    fun parse(json: String?): List<PlatformRule> {
        ...
        return try {
            ...
            (0 until arr.length()).mapNotNull { i ->
                ...
                PlatformRule(o.optString("kw", ""), links, o.optString("app", ""), apps)
            }
        } catch (t: Throwable) {
            emptyList()
        }
    ...
    fun match(ctx: Context?, text: String): PlatformMatch? {
        for (r in rules(ctx)) {
            if (keywordMatches(r, text)) {
                return PlatformMatch(r.app, r.apps)
            }
        }
        for (r in rules(ctx)) {
            for (l in r.links) {
